...
const WidgetSettings = ({ business }) => {
  ...
  const handleConfigChange = async (key, value) => {
    ...
    const response = await axiosInstance.get(
      `${API_PATHS.WIDGETS.GET_WIDGETS(key)}?layout=${value}`
    );
    localStorage.setItem("widgetConfig", JSON.stringify(newConfig));
    setWidgetConfig(response);
      ...
      <div className="bg-white p-4 sm:p-6 shadow-sm border border-gray-200">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2 p-4 mb-6 bg-green-50 text-green-800 border-l-4 border-green-500">
          <p className="font-semibold flex items-center">
            ...
            Connected to TrueTestify account:{" "}
              ...
              {layouts.map((layout) => (
                <button
                  ...
                  onClick={() =>
                    ...
                    handleConfigChange(business?.slug, layout.value)
                  }
                  disabled={!hasFeature(layout.feature)}
                  className={`px-4 py-3 w-auto font-semibold text-sm transition-colors border ${
                    widgetConfig.data?.type === layout.value
        ...
        <div className="flex items-center justify-between p-4 bg-gray-50 border border-gray-200">
          ...
          <button
            onClick={() =>
              handleConfigChange("autoplay", !widgetConfig.autoplay)
            }
        ...
        <div className="mt-6 grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div className="border border-gray-300 p-3 sm:p-4">
            ...
            <pre className="text-xs bg-gray-50 p-3 overflow-x-auto">{`<iframe src="${publicReviewBaseUrl}/public-reviews/${business?.slug}" style="width:100%;height:420px;border:0;" loading="lazy"></iframe>`}</pre>
          </div>
        </div>
        ...
        <div className="text-center mt-8 p-4 sm:p-6 bg-gray-50 border border-gray-200">
          ...
          <div className="border border-gray-300 p-3 sm:p-4">
            <p className="text-sm text-gray-500 mb-2">
              This is a live preview of how your widget would look with the
              current settings.
            </p>
            <PublicReviews isPreview={true} layout={widgetConfig.layout} />
            {!hasFeature("widget_embed") && (
              <p className="mt-4 text-sm text-red-500">
                Widget embedding is not available on your current plan.
              </p>
            )}
          </div>
        </div>
              ...
              <ul className="list-disc list-inside text-sm text-gray-600 space-y-1">
                ...
                <li>
                  ...
                  <code className="bg-gray-100 px-1">
                    [truetestify_widget type="{widgetConfig.layout}"]
                  </code>
                </li>
              </ul>
              <a
                href="https://truetestify.com"
                target="_blank"
        ...
        <h3 className="text-2xl font-bold text-gray-800 mt-8 mb-4">
          3. Offline Collection QR
        </h3>
        <p className="text-gray-600 mb-4">
          Print this QR on packaging, receipts, or displays. Scanning redirects
          customers to your public review page where they can submit a review.
        </p>
        <div className="flex flex-col items-center gap-4 p-4 sm:p-6 bg-white border border-gray-200">
          <p className="text-sm text-gray-600">
            Target URL:{" "}
            <span className="font-mono break-all">{publicRecordUrl}</span>
          </p>
          {qrDataUrl ? (
            <img
              src={qrDataUrl}
            ...
            <div className="w-48 h-48 flex items-center justify-center border border-dashed border-gray-300 text-gray-400 text-sm">
              Generating…
            </div>
          )}
          <button
            onClick={handleDownloadQr}
            className="px-6 py-2 bg-orange-500 text-white font-semibold hover:bg-orange-600"
          >
            Download QR Code
          </button>
          <button
            onClick={() => navigate(`/record/${businessSlug}`)}
            className="px-6 py-2 bg-orange-500 text-white font-semibold hover:bg-orange-600"
          >
            Leave Review
          </button>
