import { DocumentTextIcon, TrashIcon, VideoCameraIcon } from "@heroicons/react/16/solid";
...
const Moderation = () => {
  ...
  const updateReview = async (id, newStatus) => {
    ...
    await axiosInstance.patch(API_PATHS.REVIEWS.UPDATE_REVIEW(`${id}/${newStatus}`))
        ...
        {reviews.map(review => (
          <div key={review.id} className="flex flex-col sm:flex-row items-start sm:items-center bg-gray-50 p-4 border border-gray-200 justify-between transition-all duration-200 cursor-pointer hover:bg-gray-100" onClick={() => openModal(review)}>
            <div className="flex items-center space-x-4 mb-4 sm:mb-0">
              {review.videoId  && <VideoCameraIcon src={review.videoId.url} controls className="w-15 h-15 object-cover text-blue-400" onClick={(e) => e.stopPropagation()} />}
              {review.audioId  &&  <SpeakerWaveIcon src={review.audioId.url} controls className="w-15 h-15 object-cover text-orange-400" onClick={(e) => e.stopPropagation()} />}
              {review.text  && (
            ...
            <div className="flex space-x-2 mt-4 sm:mt-0" onClick={(e) => e.stopPropagation()}>
              {review.status === 'PENDING' && (
                <>
                  <button onClick={() => updateReview(review.id, 'APPROVED')} className="bg-green-600 text-white px-2 py-1 font-medium hover:bg-green-700 transition-colors">Approve</button>
                  <button onClick={() => updateReview(review.id, 'REJECTED')} className="bg-red-600 text-white px-2 py-1 font-medium hover:bg-red-700 transition-colors">Reject</button>
                </>
              )}
              {review.status !== 'PENDING' && (
