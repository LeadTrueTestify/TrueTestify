...
export class AuthService {
  ...
  async register(dto: RegisterDto) {
    ...
    await this.prisma.membership.create({
      data: { role: 'OWNER', tenantId: tenant.id, userId: user.id },
    });

    // Create billing account placeholder
    await this.prisma.billingAccount.create({
      data: { tenantId: tenant.id, stripeCustomerId: `pending_${tenant.id}` },
    });
  ...
  async login(email: string, password: string) {
    ...
    const token = await this.jwt.signAsync({ sub: user.id });
    return { token, user: { id: user.id, email: user.email }, tenant: user.memberships[0]?.tenant};
  }
}
